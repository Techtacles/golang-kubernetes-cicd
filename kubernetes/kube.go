...
func Deploy(ctx context.Context, client *kubernetes.Clientset, filename string) (map[string]string, error) {
	...
	app_file, err := os.ReadFile(filename)
	if err != nil {
...
func WaitForPods(ctx context.Context, client *kubernetes.Clientset, deployment_labels map[string]string) error {
	for {
